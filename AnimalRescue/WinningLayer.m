...
-(id)init
{
    if ((self = [super init]))
    {
        ...
        CCMenuItem *youWon2 =[CCMenuItemFont itemWithString:@"YOU WIN"];
        youWon2.position = ccp(screenSize.width/2, screenSize.height/2 + 128 - 32);


        
        CCMenuItemFont *item1 = [CCMenuItemFont itemWithString:@"Next Level" target:self selector:@selector(onNextLevel:)];
